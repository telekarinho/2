...
if ($ok && $userid) {
    ...
    require_once 'modules/payment/stripe_create_on_register.php';
    if (is_array($member)) {
        try {
            $user_data = [
                'idmbr'    => $userid,
                ...
                'username' => $username,
                'firstname'=> $firstname,
                'phone'    => $numero_corrigido, // formato internacional
                'country'  => $country, // ex: 'BR', 'US', etc.
            ];

            $response = create_stripe_connect_account($user_data);

            if (!$response['success']) {
                error_log('Stripe Connect: ' . $response['error']);
                // Opcional: exibir aviso ao admin ou registrar no banco
            }
        } catch (Exception $e) {
